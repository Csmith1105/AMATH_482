clear all; close all; clc

L=30; % time slot
n=512; % Fourier modes

t2=linspace(-L,L,n+1); 
t=t2(1:n); 
k=(2*pi/(2*L))*[0:(n/2-1) -n/2:-1]; 
ks=fftshift(k);
noise=10;

labels=['(a)';'(b)';'(c)';'(d)']; 

realize=[1 2 5 100];  % In our HW we have 20 given realizations/observations

for jj=1:length(realize)
    
    u=sech(t);  % In our HW, we only have the values of the function at discrete
    ave=zeros(1,n);  % the 
    ut=fft(u); % transforms from a func of time to a func of frequency
    
        for j=1:realize(jj)
            % add noise to signal in the frequency domain
            utn(j,:)=ut+noise*(randn(1,n)+i*randn(1,n)); 
            % cumulative sum
            ave=ave+utn(j,:);
            
            % WHAT IS GOING ON HERE?
            dat(j,:)=abs(fftshift(utn(j,:)))/max(abs(utn(j,:))); 
            un(j,:)=ifft(utn(j,:));
            
        end
        
        ave=abs(fftshift(ave))/realize(jj);
        subplot(4,1,jj)
        plot(ks,ave/max(ave),'k') 
        set(gca,'Fontsize',[15])
        axis([-20 20 0 1]) 
        text(-18,0.7,labels(jj,:),'Fontsize',[15]) 
        ylabel('|fft(u)|','Fontsize',[15])
end
hold on 
plot(ks,abs(fftshift(ut))/max(abs(ut)),'k:','Linewidth',[2]) 
set(gca,'Fontsize',[15])
xlabel('frequency (k)')